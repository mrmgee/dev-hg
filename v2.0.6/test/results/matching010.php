...
echo '<!-- <p>Matching code here</p> -->'.PHP_EOL;
		$i = 0;
	$blArr = array();  	$matchImgFidArr = array(9,2,5);  	$selArr = array();
...
$blocks = $a->getAreaBlocksArray($pageCont);
	...
	foreach ($blocks as $b) {
		...
		if ($blTypeName == "matching"){  			array_push($blArr, $i);
			...
			echo '<div id="incorrMsg'.($i+1).'" class="itemInfoCont incorrCont">'.$btc->field_7_wysiwyg_content.'<div class="clear"></div></div>'.PHP_EOL;  			
...
$trInNum = sprintf("%02d",($oddEven-1));
	...
	if( $odd = $oddEven%2 ){  		$itemMarg = 'left';  	} else {  		$itemMarg = 'right';  	}
